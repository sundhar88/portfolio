...
const DEFAULT_CONFIG: Required<Omit<GradualBlurProps, 'width' | 'hoverIntensity' | 'preset' | 'onAnimationComplete' | 'className' | 'style'>> = {
  position: 'bottom',
  strength: 5,
  height: '3rem',
  divCount: 6,
...
const useResponsiveDimension = (responsive: boolean, config: any, key: 'height' | 'width') => {
  ...
  useEffect(() => {
    ...
    const calc = () => {
      ...
      if (w <= 480 && config[`mobile${keyStr}`])
        v = config[`mobile${keyStr}`];
      else if (w <= 768 && config[`tablet${keyStr}`])
        v = config[`tablet${keyStr}`];
      else if (w <= 1024 && config[`desktop${keyStr}`])
        v = config[`desktop${keyStr}`];
      setValue(v);
    };
